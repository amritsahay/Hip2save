...
class CookBookViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        tracker.set(kGAIScreenName, value: "CookBookViewController")
    ...
    func loadStore()
    {
        ...
        let parameters:Parameters = [Key.option.rawValue:API.recipes_cat.rawValue]
        ...
        Alamofire.request(url, method: .get, parameters: parameters, encoding:URLEncoding.default, headers: nil).validate().responseString { (response) in
            ...
            case .success:
                ...
                if status == "ok"
                {
                    ...
                    self.categories = self.categories.sorted(by: { ($0["cat_title"] ) < ($1["cat_title"] ) })
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == Identifiers.CookDetailIdentifier.rawValue
        {
            let detailVC:CookBookDetailViewController = segue.destination as! CookBookDetailViewController
//            let cat_id = sender as? Int
//            let cooktitle = sender as? String
//            detailVC.category_id = cat_id
//            detailVC.cooktitle = cooktitle!
            let array = sender as? [Any]
            detailVC.category_id = array![0] as? Int
            detailVC.cooktitle = (array![1] as? String)!
        }
    }
...
extension CookBookViewController:UITableViewDelegate,UITableViewDataSource
{
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        {
            //tableView.setEmptyView(title:"There are no recipes category found.", message: "")
        }
        else
        {
            tableView.restore()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier:Identifiers.CookDetailIdentifier.rawValue, sender: [catID, cooktitle])
    }
}
