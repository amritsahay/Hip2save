...
class CookBookDetailViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        setNotification()
        self.title = (cooktitle ?? "")
        guard let tracker = GAI.sharedInstance().defaultTracker else { return }
        tracker.set(kGAIScreenName, value: "CookBookDetailViewController")
    ...
    func createParam() -> Parameters
    {
        ...
        if searchBarHeight.constant > 0
        {
            parameters = [Key.option.rawValue:Key.deal_search.rawValue,Key.current_page.rawValue:current_page,Key.posts_per_page.rawValue:batch_size,Key.keyword.rawValue:searchBar.text!,Key.recipes_catid.rawValue: category_id!,Key.deal_type.rawValue:Key.recipes.rawValue]
        }
        else
        {
            parameters = [Key.option.rawValue:Key.get_posts.rawValue,Key.current_page.rawValue:current_page,Key.posts_per_page.rawValue:batch_size,Key.recipes_catid.rawValue: category_id!,Key.deal_type.rawValue:Key.recipes.rawValue]
        }
        return  parameters
    }
...
extension CookBookDetailViewController:UITableViewDelegate,UITableViewDataSource
{
...
extension CookBookDetailViewController:UISearchBarDelegate
{
