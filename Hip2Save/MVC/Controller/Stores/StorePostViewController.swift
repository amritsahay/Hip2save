//
//  StorePostViewController.swift
//  Hip2Save
//
//  Created by ip-d on 07/02/19.
...
class StorePostViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        tracker.set(kGAIScreenName, value: "StorePostViewController")
    ...
    func createParam() -> Parameters
    {
        ...
        if searchBarHeight.constant > 0
        {
            parameters = [Key.option.rawValue:Key.deal_search.rawValue,Key.current_page.rawValue:current_page,Key.posts_per_page.rawValue:batch_size,Key.keyword.rawValue:searchBar.text!,Key.store_id.rawValue:store_id ?? 0]
        }
        else
        {
            parameters = [Key.option.rawValue:Key.get_posts.rawValue,Key.current_page.rawValue:current_page,Key.posts_per_page.rawValue:batch_size,Key.store_id.rawValue:store_id ?? 0]
        }
...
extension StorePostViewController:UITableViewDelegate,UITableViewDataSource
{
    // MARK:- TableView DataSource
    func numberOfSections(in tableView: UITableView) -> Int {
        if deals.count == 0
        {
            //tableView.setEmptyView(title:"There are no store deals found.", message: "Please swipe down to refresh.")
        }
        else
        {
            tableView.restore()
...
extension StorePostViewController:UISearchBarDelegate
{
