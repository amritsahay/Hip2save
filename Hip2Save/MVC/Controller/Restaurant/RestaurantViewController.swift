...
class RestaurantViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        loadMoreItems()
        setUI()
        if !AppDelegate.restroreloaddata{
            AppDelegate.restrocheck = true
        }else{
            AppDelegate.restroreloaddata = false
        }
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        tracker.set(kGAIScreenName, value: "RestaurantViewController")
        
        guard let builder = GAIDictionaryBuilder.createScreenView() else { return }
        tracker.send(builder.build() as [NSObject : AnyObject])
        if AppDelegate.restroreloaddata && AppDelegate.restrocheck{
            setNotification()
            deals.removeAll()
            dealListView.reloadData()
            current_page = 1
    ...
    func createParam() -> Parameters
    {
        ...
        if searchBarHeight.constant > 0
        {
            parameters = [Key.option.rawValue:Key.deal_search.rawValue,Key.current_page.rawValue:current_page,Key.posts_per_page.rawValue:batch_size,Key.keyword.rawValue:searchBar.text!,Key.deal_type.rawValue: Key.restaurant.rawValue]
        }
        else
        {
            parameters = [Key.option.rawValue:Key.get_posts.rawValue,Key.current_page.rawValue:current_page,Key.posts_per_page.rawValue:batch_size,Key.deal_type.rawValue: Key.restaurant.rawValue]
        }
...
extension RestaurantViewController:UITableViewDelegate,UITableViewDataSource
{
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        if deals.count == 0
        {
           // tableView.setEmptyView(title:"There are no restaurant deals found.", message: "Please swipe down to refresh.")
        }
        else
        {
            tableView.restore()
