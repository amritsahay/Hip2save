...
class DealsViewController: UIViewController {
    ...
    @IBOutlet weak var dealListView: UITableView! { didSet {
        self.dealListView.tableHeaderView = tableHeader
        self.dealListView.register(UINib.init(nibName:"DealsTableViewCell", bundle:nil), forCellReuseIdentifier: "DealsTableViewCell")
           self.dealListView.tableFooterView = UIView()
        self.dealListView.separatorStyle = .none
        }
    }
    
    @IBOutlet var dealPager: FSPagerView! {
        didSet {
            
            self.dealPager.register(UINib.init(nibName:"CollectionViewCell", bundle:nil), forCellWithReuseIdentifier: "cell")
            
            self.dealPager.automaticSlidingInterval = 8.0 - self.dealPager.automaticSlidingInterval
            
            self.dealPager.isInfinite = true
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        tracker.set(kGAIScreenName, value: "DealsViewController")
        
        guard let builder = GAIDictionaryBuilder.createScreenView() else { return }
        tracker.send(builder.build() as [NSObject : AnyObject])
        if AppDelegate.dealreloaddata {
            setNotification()
            current_page = 1
            deals.removeAll()
            dealListView.reloadData()
            ...
            setUI()
            api_banner()
            AppDelegate.dealreloaddata = false
        }
    ...
    func api_banner()
    {
        let parameters:Parameters = [Key.option.rawValue:Key.top_four.rawValue]
        ...
        Alamofire.request(url, method: .get, parameters: parameters, encoding:URLEncoding.default, headers: nil).validate().responseString { (response) in
            ...
            case .success:
                ...
                self.banners = json.arrayObject as! [[String : Any]]
                self.pageControl.numberOfPages = self.banners.count
                self.pageControl.currentPage = 0
                self.dealPager.reloadData()
    ...
    func createParam() -> Parameters
    {
        ...
        if searchBarHeight.constant > 0
        {
            parameters = [Key.option.rawValue:Key.deal_search.rawValue,Key.current_page.rawValue:current_page,Key.posts_per_page.rawValue:batch_size,Key.keyword.rawValue:searchBar.text!,"popular":"active"]
        }
        else
        {
            parameters = [Key.option.rawValue:Key.get_posts.rawValue,Key.current_page.rawValue:current_page,Key.posts_per_page.rawValue:batch_size,"popular":"active"]
        }
        ...
        Alamofire.request(url, method: .post, parameters: parameters, encoding:URLEncoding.default, headers: headers).validate().responseString { (response) in
            ...
            case .success:
                ...
                if status == "ok"
                {
                    ...
                    self.current_page += 1
                    //self.responseHandle(items:items!)
    ...
    /* func stupidCode(items:[JSON])
     ...
     if hasPopular
     {
     let date = Utility.shared.convertServerTime(serverDate:popular!["post_date"].stringValue)
     let newDate = headerFormatter.date(from:headerFormatter.string(from:date!))!
     holder.append([newDate : [popular!]])
     }
    ...
    /*  func responseHandle(items:[JSON])
     {
     var hasPopular = false
     var posts:[JSON] = items
     let popular = items.last
     ...
     headerFormatter.timeStyle = .none
     headerFormatter.doesRelativeDateFormatting  = false
     var formatedDictionary = Dictionary.init(grouping: posts) { (item) -> Date in
     let date = Utility.shared.convertServerTime(serverDate:item["post_date"].stringValue)
     return headerFormatter.date(from:headerFormatter.string(from:date!))!
     }
     ...
     if hasPopular
     {
     let keys = Array(formatedDictionary.keys)
     let index = keys.count > 0 ? keys.count - 1 : 0
     formatedDictionary[keys[index]]?.append(popular!)
     }
     let previousKeys = Array(tempDeals.keys)
     let currentKeys = Array(formatedDictionary.keys)
     ...
     {
     tempDeals[previousKey] = tempDeals[previousKey]! + formatedDictionary[currentKey]!
     }
     else
     {
     tempDeals[currentKey] = formatedDictionary[currentKey]
     }
     }
     }
     }
...
extension DealsViewController:FSPagerViewDataSource,FSPagerViewDelegate
{
    ...
    public func pagerView(_ pagerView: FSPagerView, cellForItemAt index: Int) -> FSPagerViewCell {
        let cell:CollectionViewCell = pagerView.dequeueReusableCell(withReuseIdentifier: "cell", at: index) as! CollectionViewCell
        
        cell.banner?.kf.indicatorType = .activity
        let imagePath = banners[index]["h2s_column_background"]
        let url = URL.init(string:imagePath as? String ?? "" )
        cell.banner?.kf.setImage(with: url, placeholder:UIImage.init(named:"placeholder-image"), options: [.cacheOriginalImage], progressBlock: nil, completionHandler:nil)
        cell.lTitle.text  = banners[index]["h2s_column_text"] as? String                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   
        return cell
    }
    ...
    func pagerView(_ pagerView: FSPagerView, didSelectItemAt index: Int) {
        ...
        let link = banners[index]["h2s_column_link"] as? String
...
extension DealsViewController:UITableViewDelegate,UITableViewDataSource
{
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        header.title.text
            = Utility.shared.convertTime(format:"MMM dd, yyyy", time:deals[section].keys.first!)
